...
4. **文体(2026-09-16更新)**: 「**太宰治型の自嘲 × 夏目漱石型の皮肉**」。三浦しをん風(温かい擬人化)から変更済み — 過去記事も含め全面的にこの文体で統一している。
   ...
   - **英語ソースからの引用は、必ず日本語に訳してから引用する(2026-09-17、ユーザー指示)**。英語の原文をそのまま「」内に入れない。自然な日本語に意訳し、リンク先(URL)は元の英語記事のまま残す。2026-09-17に既存記事内の英語引用25件(audio-interface/midi-keyboard/sampler/scarlett-2i2/scotch-whisky/turntableの6記事、DAW記事は該当なし)を日本語訳に置き換え済み。
...
5b. **アフィリエイト画像リンクの配置場所(2026-09-17制定 → 同日改訂・必須)**: **記事冒頭にまとめて並べない**。各製品の見出し(`## 製品名 — 一言`)の直後・レーダーチャート画像の前に、その製品1個ぶんだけの写真付きリンクを差し込む(5製品比較記事なら記事内5箇所に分散、単発深掘り記事なら記事冒頭の1箇所のみ)。フォーマットは `assets/css/extended/product-links.css` で定義した `.product-links`(1アイテムでも同じクラスで囲む):
   ```markdown
   ## 製品名 — 一言

   <div class="product-links">
   <a class="product-link" href="https://www.amazon.co.jp/dp/{ASIN}?tag=nakimoto1-22"><img src="/images/products/{slug}.png" alt="{製品名}"><span class="product-link-name">{製品名}</span><span class="product-link-cta">🛒 Amazonで見る</span></a>
   </div>
   ...
   - Amazonに商品が無い場合は `href` を公式サイトのURLにし、`product-link-cta` を「公式サイトで見る」にする(タグなし)。この場合、直後に「(Amazonに単体販売の取り扱いなし)」等の一行注記を添える。
