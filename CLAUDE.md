...
- **実装方式**: このClaude Codeセッション内の`CronCreate`(2時間おき)。**重要な制約(2026-09-18、ユーザーに事前説明済みでこの方式を選択)**: CronCreateのジョブはこのセッション/ウィンドウを開いている間だけ有効で、最長7日(仕組み上の上限は7〜14日)で自動的に終了する。PCを閉じる・セッションが終わる等で通知なく停止する。Windowsタスクスケジューラのような真の永続実行ではない。**7日前後で投稿が止まっていたら、ユーザーに再依頼してもらうか、`scripts/x-autopost`と同様のタスクスケジューラ方式への切り替えを検討すること。**
...
- **重複防止**: `scripts/carbonation-covered.json`にASINのリストを保持。新しい記事を選ぶ前に必ずこのファイルを確認し、公開後は選んだASINをこのファイルに追記すること(このファイルがユーザーの指示「同じ製品の記事は書かないこと」を担保する唯一の永続的な状態なので、更新を絶対に忘れないこと)。
...
- **ジョブID**: CronCreateで作成したジョブIDは都度変わるため、`CronList`で確認すること。ユーザーから「止めて」と言われたら`CronDelete`で削除する。
...
- **実装方式**: 上記の炭酸飲料タスクと同じくこのセッション内の`CronCreate`(1時間おき)。**同じ制約が適用される**: セッション/ウィンドウを閉じると通知なく停止、最長7日で自動終了。
- **投稿方法**: `scripts/x-autopost/poster.py`の`post_tweet(text, url)`をそのまま流用(OSレベルのマウス・キーボードシミュレーションでEdge経由投稿、Claude CLIのサブプロセス呼び出しは不要 — cron発火時点のClaude自身が対話ターンとして動いているので、その場でツイート文を直接考えて渡せばよい)。
...
- **運用上のリスク(2026-09-18時点でユーザーに明示的確認はしていないが記録しておく)**: 既存のブログ告知用X投稿は「週数回程度の低頻度」を前提にBot検知回避の安全策としていたが、**この新タスクは1時間に1回=1日24回という大幅に高い頻度**になる。OSレベル入力シミュレーションでCDP検知は回避できるが、投稿頻度・時間間隔の規則性自体がX側のスパム/自動化検知に引っかかるリスクは相応にある。何らかの形でアカウント制限が発生した場合はこのジョブを即座に`CronDelete`で止め、ユーザーに報告すること。
- **ジョブID**: `CronList`で確認。停止指示があれば`CronDelete`。
...
5b. **アフィリエイト画像リンクの配置場所とデザイン(2026-09-17制定 → 2026-09-18再改訂・必須)**: **記事冒頭にまとめて並べない**。各製品の見出し(`## 製品名 — 一言`)の直後・レーダーチャート画像の前に、その製品1個ぶんだけのリンクを差し込む(5製品比較記事なら記事内5箇所、10製品なら10箇所に分散)。**デザインは固定サイズの横長バナー型リンク`.product-banner`**(2026-09-18改訂: 画像のみのフルワイドリンク`.product-links`から変更。型その2の単発深掘り記事で先に採用した`.product-banner`を型その1の比較記事にも統一適用)。`assets/css/extended/product-links.css`で定義、幅468px×高さ120px固定、左に商品写真・右に商品名+「Amazonで見る」CTAテキスト:
   ...
   - **注意**: はてなブログ側にはこのCSSが効かないため、`.product-banner`の固定サイズ横長レイアウトは当たらず、画像+テキストが縦積みの簡素な表示になる(メインサイトとは見た目が異なるが機能的には問題ない)。
