...
5b. **序文直後のアフィリエイト画像リンク(2026-09-17、ユーザー指示・必須)**: 序文(冒頭の1〜2段落)を書いたら、その直後に必ず商品の**写真付きアフィリエイトリンク**を置く。フォーマットは `assets/css/extended/product-links.css` で定義した `.product-links` ギャラリー:
   ```html
   ...
   ```
   - 5製品比較記事なら5個、単発深掘り記事なら1個を横並びで入れる。
   ...
   - Amazonに商品が無い場合は `href` を公式サイトのURLにし、`product-link-cta` を「公式サイトで見る」にする(タグなし)。
   - 新しい記事を書くたびに、この節の直後にこのブロックを追加すること。
